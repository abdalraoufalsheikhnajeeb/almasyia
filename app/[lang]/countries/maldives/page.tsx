...
<div className="container mx-auto p-4 mb-8 pt-6 lg:ps-4 bg-[#34689447] rounded-2xl border-white border shadow-xl overflow-hidden">
      <div className="mb-8">
        ...
        <h2 className="text-center text-accent text-2xl font-bold mb-4 mt-8">
          {lang === "en" ? "Main Activities and Events:" : lang === "ar" ? "أهم الأنشطة والفعاليات:" : "Основные мероприятия и события:"}
        </h2>
      ...
      <h2 className="text-center text-accent text-2xl font-bold mb-4 mt-8">
        {lang === "en" ? "Best Places for Shopping:" : lang === "ar" ? "أفضل أماكن للتسوق:" : "Лучшие места для шопинга:"}
      </h2>
        ...
        <table className="min-w-full bg-white shadow-md rounded-2xl">
          <thead>
            <tr>
              ...
              <th className="px-4 py-2 text-center bg-blue-600 text-white">
                {lang === "en" ? "Description" : lang === "ar" ? "الوصف" : "Описание"}
              </th>
            </tr>
          </thead>
          <tbody>
            {Array.from({ length: 6 }).map((_, index) => (
              <tr key={index} className="border-t">
                <td className="px-4 text-center py-2">
                  {lang === "en"
                    ? ["Le Cute", "Chaandhanee Magu", "Centro Mall", "Oevaali Art Shop & Studio", "Male Local Market", "Majeedhee Magu"][index]
                    : lang === "ar"
                    ? ["لو كيوت", "شانادانا ماجو", "سنترو مول", "متجر واستوديو أوفالي للفن", "سوق ماليه المحلي", "ماجيده ماجو"][index]
                    : ["Le Cute", "Chaandhanee Magu", "Centro Mall", "Oevaali Art Shop & Studio", "Male Local Market", "Majeedhee Magu"][index]}
                </td>
                <td className="px-4 text-center py-2">
                  {lang === "en"
                    ? ["A popular gift shop", "A street with numerous souvenir shops", "A modern shopping mall", "An art shop with local crafts", "The main market for fresh produce", "A bustling street with various shops"][index]
                    : lang === "ar"
                    ? ["متجر هدايا شهير", "شارع يضم العديد من محلات الهدايا التذكارية", "مول تسوق حديث", "متجر فنون وحرف محلية", "السوق الرئيسي للمنتجات الطازجة", "شارع مزدحم بالعديد من المحلات التجارية"][index]
                    : ["Популярный сувенирный магазин", "Улица с множеством сувенирных магазинов", "Современный торговый центр", "Магазин искусств с местными ремеслами", "Основной рынок свежих продуктов", "Оживленная улица с разнообразными магазинами"][index]}
                </td>
              </tr>
            ))}
          </tbody>
      ...
      <h2 className="text-center text-accent text-2xl font-bold mb-4 mt-8">
        {lang === "en" ? "Best Restaurants:" : lang === "ar" ? "أفضل المطاعم:" : "Лучшие рестораны:"}
      </h2>
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full bg-white shadow-md rounded-2xl">
          <thead>
            <tr>
              <th className="px-4 py-2 text-center bg-blue-600 text-white">
                {lang === "en" ? "Restaurant" : lang === "ar" ? "المطعم" : "Ресторан"}
              </th>
              <th className="px-4 py-2 text-center bg-blue-600 text-white">
                {lang === "en" ? "Description" : lang === "ar" ? "الوصف" : "Описание"}
              </th>
          ...
          <tbody>
            {Array.from({ length: 6 }).map((_, index) => (
              <tr key={index} className="border-t">
                <td className="px-4 text-center py-2">
                  {lang === "en"
                    ? ["Marrybrown", "Sea House Restaurant", "Asian Wok", "Symphony Restaurant", "The Manhattan Fish Market", "Salt Cafe & Restaurant"][index]
                    : lang === "ar"
                    ? ["ماري براون", "مطعم بيت البحر", "مطعم ووك الآسيوي", "مطعم سيمفوني", "مطعم سوق سمك مانهاتن", "مطعم وكافيه سولت"][index]
                    : ["Marrybrown", "Sea House Restaurant", "Asian Wok", "Symphony Restaurant", "The Manhattan Fish Market", "Salt Cafe & Restaurant"][index]}
                </td>
                <td className="px-4 text-center py-2">
                  {lang === "en"
                    ? ["A popular fast food chain", "A seafood restaurant with ocean views", "A restaurant serving Asian cuisine", "A restaurant with live music", "A famous seafood restaurant", "A cafe and restaurant with a cozy atmosphere"][index]
                    : lang === "ar"
                    ? ["سلسلة مطاعم وجبات سريعة شهيرة", "مطعم مأكولات بحرية بإطلالة على المحيط", "مطعم يقدم المأكولات الآسيوية", "مطعم مع موسيقى حية", "مطعم مأكولات بحرية شهير", "كافيه ومطعم بأجواء مريحة"][index]
                    : ["Популярная сеть быстрого питания", "Ресторан морепродуктов с видом на океан", "Ресторан азиатской кухни", "Ресторан с живой музыкой", "Знаменитый ресторан морепродуктов", "Кафе и ресторан с уютной атмосферой"][index]}
                </td>
              </tr>
            ))}
          </tbody>
